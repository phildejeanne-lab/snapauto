  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-5">
        <Link href="/clients" className="text-sm font-medium text-brand-400 hover:underline">← Clients</Link>
        ...
        {contact && (
          <>
            <div className="mt-3 mb-6 flex items-center gap-4">
              <span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-brand-50 text-xl font-bold text-brand-300">
                {(contact.name ?? "?").trim().charAt(0).toUpperCase()}
              </span>
              <div>
                <h1 className="text-2xl font-bold tracking-tight text-slate-100">{contact.name || "Sans nom"}</h1>
            ...
            <div className="mb-6 grid gap-4 rounded-2xl border border-slate-800 bg-slate-900/60 p-5 shadow-sm sm:grid-cols-2">
            ...
            <ul className="flex flex-col gap-2">
                  ...
                  >
                    <span
                      className={`shrink-0 rounded-md px-2 py-1 text-xs font-semibold ${
                        d.operation === "achat" ? "bg-amber-500/15 text-amber-300" : "bg-emerald-500/15 text-emerald-300"
                      }`}
                    >
              ...
              {dossiers.length === 0 && <li className="text-sm text-slate-400">Aucun dossier.</li>}
            </ul>
          </>
        )}
      </main>
