  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-5">
        <h1 className="mb-1 text-2xl font-bold tracking-tight text-slate-100">Clients</h1>
        ...
        {rows && rows.length > 0 && (
          <input
            ...
            className="mb-4 w-full rounded-xl border border-slate-700 px-3.5 py-2.5 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
          />
        )}

        {error && <p className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-300">{error}</p>}
        {!rows && !error && <p className="text-sm text-slate-400">Chargement…</p>}
        {rows && rows.length === 0 && (
        ...
        <ul className="flex flex-col gap-2">
          {filtered?.map((r) => (
            <li key={r.id}>
              ...
              >
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-brand-50 text-sm font-bold text-brand-300">
